...
def invert_dense_distribution(dense_distribution):
    INVERT = []
    for value in dense_distribution:
        invert = 1/value
    ...
    for invert in INVERT:
        invert_sum += invert
    for invert in INVERT:
        invert_distribution.append(invert/invert_sum)

    return invert_distribution
...
class SuperTuxDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.transform is not None:
            ...
            img = P(self.data[idx][0])
            augment = self.transform(img)
            self.data[idx][0] = augment
...
def load_data(dataset_path, transforms = transforms.ToTensor(), num_workers=0, batch_size=128, **kwargs):
    dataset = SuperTuxDataset(dataset_path, transforms, **kwargs)
    return DataLoader(dataset, num_workers=num_workers, batch_size=batch_size, shuffle=True, drop_last=True)


def load_dense_data(dataset_path, transforms = dense_transforms.ToTensor(), num_workers=0, batch_size=32, **kwargs):
    dataset = DenseSuperTuxDataset(dataset_path, transforms, **kwargs)
    return DataLoader(dataset, num_workers=num_workers, batch_size=batch_size, shuffle=True, drop_last=True)
