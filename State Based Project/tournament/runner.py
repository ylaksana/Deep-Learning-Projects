...
class Match:
    ...
    def _g(f):
        ...
        if ray is not None and isinstance(f, (type(ray.ObjectRef), ray._raylet.ObjectRef)):
            return ray.get(f)
        return f
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--training', action='store_true', help="Do you want to generate training data?")
    ...
    if args.parallel is None or remote.ray is None:
        ...
        # Will always run 5 matches, one for each ball location
        for i, ball_location in enumerate(ball_locations):
            recorder = None
            if args.record_video:
